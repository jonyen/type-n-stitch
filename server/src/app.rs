...
use crate::{assets, auth, library, mcp, ops, projects, routes, sources, tokens, ws, AppState};
...
pub fn router(state: Arc<AppState>) -> Router {
    Router::new()
        ...
        .route("/api/tokens", get(tokens::index).post(tokens::create))
        .route("/api/tokens/{id}", axum::routing::delete(tokens::delete))
        ...
        .route("/api/projects/{id}/ws", get(ws::handler))
        .route(
        ...
        .route(
            "/api/projects/{id}/assets/{aid}",
            axum::routing::delete(assets::delete),
        )
